...
# Nota: recuperacion de arbol MPTT corrupto -- solo se invoca cuando el rebuild
# parcial falla (fallo de integridad de datos). No alcanzable via HTTP black-box
# sin corromper deliberadamente la base de datos; no mapea a ningun RF de FN2/FN3.
@tracer.start_as_current_span('rebuild_stock_items')
def rebuild_stock_items():  # pragma: no cover
...
def rebuild_stock_item_tree(tree_id: int, rebuild_on_fail: bool = True) -> bool:
    ...
    if tree_id:
        try:
            ...
            # This is a critical error, explicitly report to sentry
            # Nota: rama de error de rebuild de arbol MPTT -- misma exclusion por
            # alcance que rebuild_stock_items (requiere corrupcion de datos real).
            report_exception(e)
...
# invoque; no mapea a ningun RF de FN2/FN3. Mismo criterio que en order/tasks.py.
@tracer.start_as_current_span('delete_old_stock_tracking')
@scheduled_task(ScheduledTask.DAILY)
def delete_old_stock_tracking():  # pragma: no cover
