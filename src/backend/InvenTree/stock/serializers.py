...
class SerializeStockItemSerializer(serializers.Serializer):
    ...
    def validate_quantity(self, quantity):
        ...
        if quantity < 0:  # pragma: no cover
            # Nota: el campo 'quantity' ya declara min_value=0, DRF rechaza antes.
    ...
    def validate(self, data):
        ...
        if not item:  # pragma: no cover
            # Nota: validate_quantity() ya lanza "No stock item provided" antes,
            # porque 'quantity' es required y se valida primero (rama sombreada).
            raise ValidationError(_('No stock item provided'))
    ...
    def save(self) -> list[StockItem]:
        ...
        if not item:  # pragma: no cover
            # Nota: misma rama sombreada por validate_quantity()/validate().
            raise ValidationError(_('No stock item provided'))
...
class InstallStockItemSerializer(serializers.Serializer):
    ...
    def validate_quantity(self, quantity):
        ...
        if quantity < 1:  # pragma: no cover
            # Nota: el campo 'quantity' ya declara min_value=1, DRF rechaza antes.
...
class StockAdjustmentItemSerializer(serializers.Serializer):
    ...
    def validate_quantity(self, quantity):
        ...
        if quantity < 0:  # pragma: no cover
            # Nota: el campo 'quantity' ya declara min_value=Decimal(0), DRF rechaza antes.
